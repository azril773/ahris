...
def readcuti(r):
    with open("static/jamkerja.json","rb") as f:
        data = json.loads(f.read())
        # print([dt for dt in data["data"] if dt["tgl_aktif"] is None])
        # pegawai_db.objects.using(r.session["ccabang"]).bulk_update([pegawai_db(id=dt["id"],tgl_masuk=datetime.strptime(dt["tgl_aktif"],"%Y-%m-%d %H:%M:%S").date()) for dt in data["data"] if dt["tgl_aktif"] is not None],["tgl_masuk"])
        # idstr = ''
        # for dt in data["data"]:
        #     idstr += str(dt["id"])+","
        # print(idstr)
        ...
        #         j.delete(using=r.session["ccabang"])
        #     else:
        #         create.append(jamkerja_db(kk_id=j.kk_id,jam_masuk=j.jam_masuk,jam_pulang=j.jam_pulang,lama_istirahat=j.lama_istirahat,hari=j.hari,add_by=j.add_by,edit_by=j.edit_by,add_date=j.add_date,edit_date=j.edit_date,shift_id=1))
        #         j.delete(using=r.session['ccabang'])
        #         continue
        # print(create)
